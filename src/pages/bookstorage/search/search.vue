...
<div class="wrapper">
    <div class="header flex flex-box">
        <div class="search-area">
            <input type="text" :placeholder="keyName" class="search-input" v-focus="true" v-model="value" @input="typing" v-on:keyup.13="keySearch"/> 
        </div>
    ...
    <div class="search"  v-infinite-scroll="load" infinite-scroll-disabled="loading" infinite-scroll-distance="5">
      <div class="searchContent" v-if="searchPage">
        ...
        <div class="AllLook">
            ...
            <normal-book v-for="(item,$index) in allLook" :key="$index" :tclass="item.tclass" :cover="item.cover" :bid="item.id" :author="item.author" :intro="item.Introduction.replace(/&nbsp;/g,' ')" :title="item.booktitle"></normal-book>
        </div>
      </div>
      <div class="search-result" v-if="searchMore">
          ...
          <div class="search-result-list">
              <div class="search-result-item">
                  <normal-book v-for="(item,$index)  in bookList" :key="$index" :author="item.author" :intro="item.Introduction" :title="item.booktitle" :cover="item.cover" :bid="item.id" :tclass="item.tclass"></normal-book>
              </div>
          </div>
...
.active {
  color: #f76c5b;
  border-bottom: 2px solid #f76c5b;
}
.search-result-list {
  background-color: #fff;
}
