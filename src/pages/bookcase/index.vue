<template>
  <div>
    ...
    <ul class="title flex">
      <p class="line"></p>
       <li v-bind:key="index" :class="{active:$route.params.type==index}" v-for="(title,index) in titles" class="bookItem" @click="selectTabItem(index)">
         {{title}}
         <p class="lines" v-show="$route.params.type==index"></p> 
       </li>
    </ul>
    <!-- 底部切换 -->
    <ul class="bookitems" 
...
.lines {
  width: 1rem;
  height: 4px;
  border-bottom: 1px solid #F76C5B;
  position: absolute;
  left: 45%;
  bottom: 20%;
}
