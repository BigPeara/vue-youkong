 ...
 <mt-loadmore class="wrapper" :top-method="loadTop" :autoFill="true" ref="reload" @top-status-change="handleTopChange">   
    <div class="header flex">
      ...
      <div class="user-info flex flex-box">
        ...
        <p v-if="showPay" class="account-info">余额：<span>{{userData.vipMoney}}</span>悠空币+<span>{{userData.readcoins}}</span>阅读币</p>
        </div>
      </div>
    </div>
    <ws-cell-module>
      <ws-cell title="充值" titleColor="#F25449" label="充值有礼" icon="amount" iconColor="#F25449" link='/pay' v-if="showPay"></ws-cell>
      <!-- <ws-cell title="包月" titleColor="#FF5480" icon="king" iconColor="#FF5480" link='/pay' v-if="showPay"></ws-cell> -->
    </ws-cell-module>
    <ws-cutoff-rull />
    <ws-cell-module hasBorder='true'>
      ...
      <ws-cell v-if="showPay" title="联系客服" icon='kefu' iconColor='#F02639' link='/mine/help/custService' ></ws-cell>
